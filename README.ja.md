...
🍥 **[new-api](https://github.com/QuantumNous/new-api) を基盤とするインテリジェント LLM ゲートウェイ — 統一 API、ゲートウェイ対応 Agent、多言語のモデル情報**
...
**NextRouter** はインテリジェント LLM ゲートウェイであり、[`QuantumNous/new-api`](https://github.com/QuantumNous/new-api) のコミュニティ Fork として上流の `main` を継続的に追跡します。new-api が提供するすべての機能—統一 API の背後での 40 以上の上流 AI プロバイダー（OpenAI、Claude、Gemini、Azure、AWS Bedrock など）の集約、ユーザー管理、課金、レート制限、管理ダッシュボード—を維持しながら、ゲートウェイの知識をモデル支援ワークフローへ接続します。ブラウザ側 Agent は、ゲートウェイが公開するモデルと管理者が整備したドキュメントを参照し、モデル、エンドポイント、導入方法に関する質問に回答できます。また管理者は LLM を使い、動的なモデル説明とドキュメントの翻訳を生成して永続化できます（[新機能](#-nextrouter-の新機能) を参照）。
...
> 以下は上流との主な差分であり、すべてを網羅しているとは限りません。各リリースの完全な変更内容は [GitHub Releases](https://github.com/WebAgentLoop/nextrouter/releases) を、上流の機能については公式ドキュメントを参照してください。
...
- **設定可能なモデル広場の表示制御**：管理者は、有効なモデル設定（完全一致または名前ルール一致）があるモデルだけをカタログに表示でき、チャネルルーティングには影響しません。
- **Markdown モデルドキュメント**：管理者はモデルごとのガイドを整備でき、カタログに表示するとともにゲートウェイ対応 Agent から参照できます。
- **モデルコンテンツの永続翻訳**：管理者がモデル説明とドキュメントの LLM 翻訳を手動で開始し、翻訳、ソースハッシュ、状態、エラーを言語ごとに保存します。ソース言語/デフォルト言語の設定と古いコンテンツの検出にも対応します。
