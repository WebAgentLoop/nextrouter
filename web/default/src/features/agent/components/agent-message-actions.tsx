...
import { AgentMessageActionButton } from './agent-message-action-button'
...
export function AgentMessageActions({
  ...
  const isLoading = Boolean(message.isStreaming)
  ...
  if (isAssistant && hasContent && !isLoading && onToggleSource) {
    actions.push({
      ...
      onClick: () => onToggleSource(message),
    })
  }
  ...
  if ((isAssistant || isUser) && hasContent && !isLoading && onRegenerate) {
    actions.push({
      ...
      onClick: guard(() => onRegenerate(message)),
    })
  }

  if (hasContent && !isLoading && onEdit) {
    actions.push({
      disabled: isGenerating,
      icon: Edit,
      label: MESSAGE_ACTION_LABELS.EDIT,
      onClick: guard(() => onEdit(message)),
    })
  }
  ...
  if (onDelete) {
    actions.push({
      ...
      onClick: guard(() => onDelete(message)),
      variant: 'destructive',
    })
  }
